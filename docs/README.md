...
- `+A`		= this does nothing but it's valid				        (ex. `+9 + 5 == 14`)
...
Scientific notation is supported for all numeric types, as well as underscore digit separators, like so: `931_121_912.129_121_192E-15`.
The `E` can be `e` or `E`, and a `+` sign is optional for positive exponents. Underscores may be placed anywhere, but only one is allowed in a row.
...
MML supports complex numbers as well. A complex number can be defined as: `z = 5 + 3i`, where `i` is a built-in constant
(see [Built-ins](#built-ins) for other built-in constants) that is multiplied by the number `3` and added to `5`.
This was not made possible via any special syntax---really, under the hood, MML just promotes a number to a complex number
if it used in an expression with a complex number, so all the example really does is promote `3` to the complex number `3+0i`,
multiply it by `i` to create `0+3i`, and add `5` to make `5+3i`, which is assigned to `z` (sort of).

Additionally, built-in functions are provided, ~~though user-defined functions are not yet implemented~~ and user-defined functions also exist. Functions are called via this syntax:
...
There is a (somewhat confusing) distinction between 'builtin' functions, which are prefixed by `@` (like `@dbg{expr}`), and functions that are
built-in, which includes all 'builtin' functions, as well as the rest of the functions provided by default.
A full catalogue of all provided built-in functions and their properties is available in [Built-ins](#built-ins).

User-defined functions are declared like so: `C{F} = (F - 32) * 5/9`, and called like a normal function: `C{80} == 26 + 2/3`.
Their names can be any valid identifier, and will take priority over built-in functions if their names overlap. If a variable
is defined with the same identifier as a user-defined function, you can't call the function (an error message will be displayed).

## <span id="built-ins">Built-ins</span> [↩](#contents)
### Built-in constants:
(these may have an additional `@` prefix, but it isn't necessary)
...
- `logb{a, b}` = returns the base `b` logarithm of `a` (either can be real or complex).
